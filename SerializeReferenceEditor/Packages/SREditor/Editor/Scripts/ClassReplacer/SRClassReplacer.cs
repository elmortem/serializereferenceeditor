...
namespace SerializeReferenceEditor.Editor.ClassReplacer
{
    ...
    {
        private string oldTypeFullName;
        private string newTypeFullName;
        ...
        private void OnGUI()
        {
            ...
            using (var scrollView = new EditorGUILayout.ScrollViewScope(scrollPosition))
            {
                ...
                using (new EditorGUILayout.HorizontalScope())
                {
                    ...
                    oldTypeFullName = EditorGUILayout.TextField(oldTypeFullName);
                    if (GUILayout.Button("Select", GUILayout.Width(60)))
                    {
                        ShowTypeSelector((selectedType) => oldTypeFullName = GetTypeFullName(selectedType));
                    }
                }
                ...
                using (new EditorGUILayout.HorizontalScope())
                {
                    ...
                    newTypeFullName = EditorGUILayout.TextField(newTypeFullName);
                    if (GUILayout.Button("Select", GUILayout.Width(60)))
                    {
                        ShowTypeSelector((selectedType) => newTypeFullName = GetTypeFullName(selectedType));
                    }
                }

                if (GUILayout.Button("Replace"))
                {
                    if (string.IsNullOrEmpty(oldTypeFullName) || string.IsNullOrEmpty(newTypeFullName))
                    {
                        statusMessage = "Error: Please select both types";
                        return;
                    }

                    if (EditorUtility.DisplayDialog("Confirm Replace", 
                        $"Replace all instances of {oldTypeFullName} with {newTypeFullName}?", 
                        "Yes", "No"))
                    {
                        ReplaceInAllAssets(oldTypeFullName, newTypeFullName);
                    }
                }
        ...
        private bool IsTypeSerializable(Type type)
        {
            ...
            if (!type.IsClass || type.IsAbstract || type.IsInterface) return false;
            if (type.Name.Contains("<>")) return false;
            if (type.IsGenericType || type.ContainsGenericParameters) return false;
            if (type.Namespace?.StartsWith("System") == true || type.IsSubclassOf(typeof(ScriptableObject))) return false;
        ...
        private void ReplaceInAllAssets(string oldTypeFullName, string newTypeFullName)
        {
            ...
            finally
            {
                ...
                statusMessage = $"Processed {processedCount} files with changes.";
                Repaint();
            }
        }
